...
impl DefMap {
    pub fn new(expn: &Expansion) -> DefMap {
        let mut defmap = DefMap {
            table: BTreeMap::new(),
        };

        expn.visit(&mut DefMapVisitor {
            defmap: &mut defmap,
            qualifier: vec!(),
        });
    ...
    fn visit_pat(&mut self, pat: &Pat) {
        match pat.val {
            IdentPat(ref ident, ref t) => {
                self.defmap.table.insert(ident.id, Def::PatDef(t.as_ref().map(|t| t.clone())));
            }
        ...
        match item.val {
            FuncItem(ref ident, ref args, ref t, ref def, ref tps) => {
                let arg_def_ids = args.iter().map(|arg| {
                    self.defmap.table.insert(arg.ident.id, Def::FuncArgDef(arg.argtype.clone()));
                    arg.ident.id
                }).collect();

                let tp_def_ids = tps.iter().map(|tp| {
                    self.defmap.table.insert(tp.id, Def::GenericDef);
                    tp.id
                }).collect();
                ...
                self.defmap.table.insert(ident.id, Def::FuncDef(arg_def_ids, t.clone(), abi, tp_def_ids));
            ...
            StructItem(ref ident, ref fields, ref tps) => {
                ...
                self.defmap.table.insert(ident.id, Def::StructDef(qn, field_map, tp_def_ids));
            },
            EnumItem(ref ident, ref variants, ref tps) => {
                let variant_def_ids = variants.iter().map(|variant| {
                    ...
                    self.defmap.table.insert(variant.ident.id, Def::VariantDef(qn, ident.id, args));
                    variant.ident.id
                }).collect();
                ...
                let qn = self.make_qualified_name(ident.val.name);
                self.defmap.table.insert(ident.id, Def::EnumDef(qn, variant_def_ids, tp_def_ids));
            }
            TypeItem(ref ident, ref t, _) => {
                self.defmap.table.insert(ident.id, Def::TypeDef(t.clone()));
            }
            ModItem(ref ident, ref module) => {
                let item_ids = module.val.items.iter().map(|item| item.id).collect();
                let qn = self.make_qualified_name(ident.val.name);
                self.defmap.table.insert(ident.id, Def::ModDef(qn, item_ids));
            ...
            StaticItem(ref ident, ref ty, ref expr, _) => {
                self.defmap.table.insert(ident.id, Def::PatDef(Some(ty.clone())));
            ...
            ConstItem(ref ident, ref ty, ref expr) => {
                self.defmap.table.insert(ident.id, Def::ConstDef(ty.clone()));
                self.visit_type(ty);
                ...
                match import.val.import {
                    ...
                    ImportNames(ref names) => {
                        for ident in names.iter() {
                            ...
                            self.defmap.table.insert(ident.id, Def::UseDef(qn, path_qn));
                        }
                    }
                }
            }
